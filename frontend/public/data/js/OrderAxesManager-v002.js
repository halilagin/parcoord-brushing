...
// if omitteds is empty, it will get highest one by no dependencies. this will be the first highest one.
  ...
  this.makeUnique = function(sequence) {
    ...
    var isExistInUnique = function(variable){
      for (var i=0;i<uniques.length;i++){
          if (uniques[i].variable==variable)
            return true;
      }
      return false;
    ...
    sequence.forEach(( item, idx)=>{
      ...
      if (isExistInUnique(item.variable)==false && uniques.length<=this.variables.length)
...
var OrderAxesManager=function (){
  ...
  this.corrKeySequence=[]; //the ordered dimensions acording to corr value. format: [{variable:f1,value:0.7}]
  ...
  this.htmls={
    ...
    "DialogFeatureBucket":`
        <div class="DialogFeatureBucket" title="Feature Bucket">
          <div>
               <!--<div class="OA_buttons">-->
                <!--<span class="OA_button OA_fb_btn_remove_item ui-button-icon ui-icon ui-icon-closethick" title="Remove variable"></span>-->
               <!--</div>-->
              <div class="OA_fb_features">
                  <select multiple="multiple" size="10" class="OA_fb_features_list" style="width: 150px !important; height: 200px !important;"></select>
              </div>
              <br style="clear: left;" />
  ...
  this.orderCorrByAlg1 = function(variables){
    ...
    this.corrKeySequence = this.orderCorrAlg.start();
    return this.corrKeySequence;
    ...
    setTimeout(
      ()=> {
        ...
        newSequence.forEach((item)=>{
          $(".OA_axeslist").append(`<option value="${item.variable}">${item.variable}</option>`);
        });
      }, 1000);
  ...
  this.saveFeaturesInAxes=function(){
    var newSequence = [];

    $(".OA_fb_features_list option").each((idx,opt)=>{
      newSequence.push($(opt).attr("value"));
  ...
  this.removeSelectedFeatures = function(){
    var isSelected = $(".OA_fb_features_list").find("option:selected").length;
    if (isSelected==0){
      console.log("Please select a variable.");
      return;
    }

    $(".OA_fb_features_list").find("option:selected").remove();
  ...
  this.showDialogOrderAxes = function(){
    ...
    $(".OA_button_select_and_saveorder").click(()=>{
      this.selectAndSaveDimensionsOrder();
    });
  ...
  this.showFeatureBucket=function(dimName){
    var saveFeaturesInAxes_ = this.saveFeaturesInAxes;
    var removeSelectedFeatures_ = this.removeSelectedFeatures;
    console.log("showFeatureBucket:show");
    $(".DialogFeatureBucket").dialog("destroy");
    $(".DialogFeatureBucket").remove();
    $("body").append(this.htmls.DialogFeatureBucket);


    $(".OA_fb_btn_saveorder").click(()=>{
      this.saveDimensionsOrder();
    });


    // $(".OA_fb_btn_remove_item").click(()=>{
    //   this.removeSelectedFeatures();
    // });


    $(".DialogFeatureBucket").dialog({
      autoOpen:false,
      width:200,
      height:300,
      buttons:{
    ...
    $(".DialogFeatureBucket").dialog("open");
  };


  this.selectFeature=function(dimName){
    $(".OA_fb_features_list").append(`<option value="${dimName}">${dimName}</option>`);
  };
