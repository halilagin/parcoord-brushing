...
class DensityPlotWithDxHistogram(object):
    '''
    classdocs
    '''
    ...
    def getData(self, mu,sigma):
        ...
        samples = []
        dists=[]
        for i in range(1): # iteratively draw samples
    ...
    def start(self, mu, sigma, filename, saveFile=True):
        ...
        plt.hist(x_, bins=20, normed=True, alpha=0.5)
...
mu = [0]
sigma = [4]
mmp = DensityPlotWithDxHistogram()
mmp.start(mu,sigma, "density-plot-with-dx-histogmram.svg")
# 
