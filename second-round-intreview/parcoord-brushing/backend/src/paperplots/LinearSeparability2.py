...
class LinearSeparability2(object):
    '''
    classdocs
    '''
    ...
    def start(self, mu, sigma, filename, saveFile=True):
        ...
        df = pd.DataFrame({'x':x_, 'y':y_})
        sns.regplot('x', 'y',df, fit_reg=False, ax=ax, color='r', marker='x', scatter_kws={"s": 25})
...
mu = [5, 12]
sigma = [3.5, 3.5]
mmp = LinearSeparability2()
mmp.start(mu,sigma, "linear-separability2.svg")
# 
